$(function() {
    ...
    $('.semispoiler .semispoiler__trigger').click(function(e) {
        var spoiler = $(this).parent();
        var content = spoiler.find('.semispoiler__content[data-minheight][data-expanded]');
        var expanded = parseInt(content.attr('data-expanded'));
        var minHeight = parseInt(content.attr('data-minheight'));
        if (expanded == 1) {
            content.css('height', minHeight + 'px');
			content.attr('data-expanded', 0);
        } else {
            content.css('height', 'auto');
			content.attr('data-expanded', 1);
        }
    });
